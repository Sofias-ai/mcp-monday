...
MONDAY_API_URL, MONDAY_API_KEY, MONDAY_BOARD_ID = "https://api.monday.com/v2", os.getenv("MONDAY_API_KEY"), os.getenv("MONDAY_BOARD_ID")
...
class BoardSchema:
    def __init__(self):
        self.schema, self.columns_config, self.columns = None, {}, {}
        self.last_update = None
        self._handlers = COLUMN_TYPE_HANDLERS

    async def initialize(self) -> None:
        if not MONDAY_API_KEY or not MONDAY_BOARD_ID: raise ValueError("Missing API_KEY or BOARD_ID")
        
        query = """query { boards(ids: %s) { name, board_folder_id, board_kind, columns { id, title, type, 
                settings_str, width, archived, description }, groups { id, title, color, position }, 
                tags { id, name, color }, owner { id, name }, permissions, state, workspace_id, 
                board_folder_id } }""" % MONDAY_BOARD_ID
                ...
                if "data" in data and "boards" in data["data"]:
                    ...
                    for col in board["columns"]:
                        ...
                        self.columns_config[col["id"]] = {
                            "title": col["title"], "type": col["type"], "settings": settings,
                            "width": col.get("width"), "archived": col.get("archived", False),
                            "description": col.get("description", "")
                        }
                        
                        col_format = ColumnFormat(column_type=col["type"], settings=settings)
                        self.columns[col["id"]] = ColumnDefinition(id=col["id"], title=col["title"],
                                                                format=col_format,
    ...
    def get_column_format(self, column_id: str) -> Dict[str, Any]:
        ...
        if column_id not in self.columns: raise ValueError(f"Unknown column ID: {column_id}")
        column = self.columns[column_id]
        return {
            "type": column.format.column_type,
            "settings": column.format.settings,
            "validation_rules": self._get_validation_rules(column.format.column_type, column.format.settings)
        }
    ...
    def format_value(self, column_id: str, value: Any) -> str:
        ...
        if not handler: raise ValueError(f"No handler for column type: {column.format.column_type}")
    ...
    def validate_column_value(self, column_id: str, value: Any) -> ValidationResult:
        ...
        if column_id not in self.columns:
            return ValidationResult(is_valid=False, message=f"Unknown column ID: {column_id}")
        ...
        try:
            ...
            return ValidationResult(is_valid=False, message=f"Validation error: {str(e)}")

    def get_columns_info(self) -> Dict[str, Any]:
        """Get complete board schema and columns information"""
        return {
            "board_info": {
                "name": self.schema.get("name"), "board_kind": self.schema.get("board_kind"),
                "workspace_id": self.schema.get("workspace_id"), "state": self.schema.get("state"),
            ...
            "columns": [
                {
                    "id": col_id, "title": info.title, "type": info.format.column_type,
                    "settings": info.format.settings, "width": self.columns_config[col_id].get("width"),
                    "archived": self.columns_config[col_id].get("archived", False),
                    "description": self.columns_config[col_id].get("description", ""),
                    "validation_rules": self._get_validation_rules(info.format.column_type, info.format.settings)
                }
